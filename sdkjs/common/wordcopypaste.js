...
(
    ...
    function(window, undefined) {
        ...
        function GetObjectsForImageDownload(aBuilderImages, bSameDoc) {
            ...
            return {
                aUrls: aUrls,
                aBuilderImagesByUrl: aBuilderImagesByUrl
            };
        }
        ...
        function sendImgUrls(api, images, callback, bExcel, bNotShowError, token) {
            if (window["NATIVE_EDITOR_ENJINE"] === true && window["IS_NATIVE_EDITOR"] !== true) {
                ...
                for (var i = 0; i < images.length; i++) {
                    ...
                    AscCommon.g_oDocumentUrls.addUrls({
                        _local_url: _full_path
                    });
                    _data[i] = {
                        url: _full_path,
                        path: _local_url
        ...
        PasteProcessor.prototype = {
            ...
            _setSpecialPasteShowOptionsPresentation: function(props) {
                ...
                specialPasteShowOptions.setFixPosition({
                    x: x,
                    y: y,
                    pageNum: curPage,
                    w: w,
                ...
                var getRtfImg = function(sRtf) {
                    ...
                    for (var i = 0; i < sRtf.length; i++) {
                        if (rg_rtf.test(sRtf[i])) {
                            ...
                            res.push({
                                data: sRtf[i].replace(rg_rtf, "").replace(/[^\da-fA-F]/g, ""),
                                type: type
                            })
                        }
                    }
            ...
            _pasteBinaryFromExcelToPresentation: function(base64FromExcel) {
                ...
                if (aContentExcel && aContentExcel.aWorksheets && aContentExcel.aWorksheets[0] && aContentExcel.aWorksheets[0].Drawings && aContentExcel.aWorksheets[0].Drawings.length) {
                    ...
                    var selectedElement, element, pDrawings = [],
                        drawingCopyObject;
            ...
            _pasteBinaryFromWordToPresentation: function(base64FromWord) {
                ...
                var elements = [],
                    selectedElement, element, drawings = [],
                    pDrawings = [],
                    drawingCopyObject;
                var defaultTableStyleId = presentation.DefaultTableStyleId;
            ...
            _readPresentationSelectedContent: function(stream, bDuplicate) {
                return AscFormat.ExecuteNoHistory(function() {
                    ...
                    return {
                        content: presentationSelectedContent,
                        fonts: fonts,
                        images: arr_Images
                    };
                }, this, []);
            ...
            _getClassBinaryFromHtml: function(node, onlyBinary) {
                var classNode, base64FromExcel = null,
                    base64FromWord = null,
            ...
            _convertTableFromExcel: function(aContentExcel) {
                ...
                for (var i = 0; i <= diffRow; i++) {
                    ...
                    for (var j = 0; j <= diffCol; j++) {
                        ...
                        table.Selection.Data[table.Selection.Data.length] = {
                            Cell: j,
                            Row: i
                        };
                        ...
                        for (var n = 0; n < value2.length; n++) {
                            ...
                            if (null != oFontItem && null != oFontItem.Name) {
                                oCurRun.Pr.RFonts.Ascii = {
                                    Name: oFontItem.Name,
                                    Index: oFontItem.Index
                                };
                                oCurRun.Pr.RFonts.HAnsi = {
                                    Name: oFontItem.Name,
                                    Index: oFontItem.Index
                                };
                                oCurRun.Pr.RFonts.CS = {
                                    Name: oFontItem.Name,
                                    Index: oFontItem.Index
                                };
                                oCurRun.Pr.RFonts.EastAsia = {
                                    Name: oFontItem.Name,
                                    Index: oFontItem.Index
                                };
                            }
                ...
                for (var i = 0; i <= diffRow; i++) {
                    for (var j = 0; j <= diffCol; j++) {
                        ...
                        for (var n = 0; n < value2.length; n++) {
                            ...
                            var oFontItem = this.oFonts[font_family];
                            if (null != oFontItem && null != oFontItem.Name) {
                                oCurRun.Set_RFonts_Ascii({
                                    Name: oFontItem.Name,
                                    Index: oFontItem.Index
                                });
                                oCurRun.Set_RFonts_HAnsi({
                                    Name: oFontItem.Name,
                                    Index: oFontItem.Index
                                });
                                oCurRun.Set_RFonts_CS({
                                    Name: oFontItem.Name,
                                    Index: oFontItem.Index
                                });
                                oCurRun.Set_RFonts_EastAsia({
                                    Name: oFontItem.Name,
                                    Index: oFontItem.Index
                                });
                            }
            ...
            _set_pPr: function(node, Para, pNoHtmlPr) {
                ...
                if (computedStyle) {
                    ...
                    if (font_family && "" != font_family) {
                        ...
                        if (null != oFontItem && null != oFontItem.Name && Para.TextPr && Para.TextPr.Value && Para.TextPr.Value.RFonts) {
                            Para.TextPr.Value.RFonts.Ascii = {
                                Name: oFontItem.Name,
                                Index: oFontItem.Index
                            };
                            Para.TextPr.Value.RFonts.HAnsi = {
                                Name: oFontItem.Name,
                                Index: oFontItem.Index
                            };
                            Para.TextPr.Value.RFonts.CS = {
                                Name: oFontItem.Name,
                                Index: oFontItem.Index
                            };
                            Para.TextPr.Value.RFonts.EastAsia = {
                                Name: oFontItem.Name,
                                Index: oFontItem.Index
                            };
                        }
                ...
                if (pagination) {
                    ...
                    else if (-1 !== pagination.indexOf("none") && -1 !== pagination.indexOf("lines-together")) {; //pPr.WidowControl = !Def_pPr.WidowControl;
                        Para.Set_KeepLines(true);
                    }
                }
                //todo KeepNext
                    ...
                    if (bPresentation) {
                        ...
                        if ("a" === sChildNodeName) {
                            ...
                            if (null != href) {
                                ...
                                if (isPasteHyperlink) {
                                    var HyperProps = new Asc.CHyperlinkProperty({
                                        Text: text,
                                        Value: href,
                                        ToolTip: title
                                    });
