    ...
    function(window, undefined) {
        ...
        function asc_CChartBinary(chart) {
            ...
            if (chart && chart.getObjectType() === AscDFH.historyitem_type_ChartSpace) {
                var writer = new AscCommon.BinaryChartWriter(new AscCommon.CMemory(false)),
                    pptx_writer;
                ...
                if (!oSparklineGroup.displayXAxis) {
                    ...
                    if (val_ax_props.minVal == null) {
                        if (aMinPoints) {
                            ...
                            aSeriesPoints.sort(function(a, b) {
                                return a.val - b.val;
                            });
                            bSorted = true;
        ...
        function DrawingObjects() {
            ...
            DrawingBase.prototype.checkBoundsFromTo = function() {
                ...
                if (_t.isGraphicObject() && _t.graphicObject.bounds) {
                    ...
                    var fromX = mmToPx(bounds.x > 0 ? bounds.x : 0),
                        fromY = mmToPx(bounds.y > 0 ? bounds.y : 0),
                        toX = mmToPx(bounds.x + bounds.w),
                        toY = mmToPx(bounds.y + bounds.h);
                    if (toX < 0) {
                        toX = 0;
                    }
                    if (toY < 0) {
            ...
            _this.init = function(currentSheet) {
                ...
                _this.canEdit = function() {
                    return worksheet.handlers.trigger('canEdit');
                };
         ...
         *****************************************/
            ...
            _this.addMath = function(Type) {
                if (_this.canEdit() && _this.controller) {
                    ...
                    _this.objectLocker.checkObjects(function(bLock) {
                        ...
                        var activeCell = worksheet.model.selectionRange.activeCell;
                        var coordsFrom = _this.calculateCoords({
                            col: activeCell.col,
                            row: activeCell.row,
                            colOff: 0,
                            rowOff: 0
                ...
                if (chart instanceof Asc.asc_ChartSettings) {
                    ...
                    window["Asc"]["editor"]._loadFonts(font_map,
                        function() {
                            ...
                            var series = oNewChartSpace.getAllSeries(),
                                ser;
            ...
            _this.drawSparkLineGroups = function(oDrawingContext, aSparklineGroups, range, offsetX, offsetY) {
                ...
                if (oDrawingContext instanceof AscCommonExcel.CPdfPrinter) {} else {
                            ...
                            if (oGraphicObject) {
                                if (drawingObject.Type === AscCommon.c_oAscCellAnchorType.cellanchorTwoCell &&
                                    ...
                                    if (drawingObject.editAs === AscCommon.c_oAscCellAnchorType.cellanchorAbsolute) {
                                        aObjectsForCheck.push({
                                            object: drawingObject,
                                            coords: drawingObject._getGraphicObjectCoords()
                                        });
                                    } else {
            ...
            _this.getDrawingCanvas = function() {
                return {
                    shapeCtx: api.wb.shapeCtx,
                    shapeOverlayCtx: api.wb.shapeOverlayCtx,
                    autoShapeTrack: api.wb.autoShapeTrack,
                    trackOverlay: api.wb.trackOverlay
                };
            };
