...
(function(window, undefined) {
            ...
            if (callback) {
                ...
                this._lockCallbacksErrorTimerId[idLockInArray] = window.setTimeout(function() {
                    if (t._lockCallbacks.hasOwnProperty(idLockInArray)) {
                        ...
                        t._lockCallbacks[idLockInArray]({
                            error: 'Timed out'
                        });
    ...
    DocsCoApi.prototype.saveChanges = function(arrayChanges, currentIndex, deleteIndex, excelAdditionalInfo, reSave) {
        ...
        this._send({
            ...
            'releaseLocks': this.canReleaseLocks,
            'reSave': reSave
        });
    };
    ...
    DocsCoApi.prototype._onForceSaveStart = function(data) {
        ...
        if (code === c_oAscServerCommandErrors.NoError) {
            ...
            this.onForceSave({
                type: c_oAscForceSaveTypes.Button,
                start: true
            });
                    ...
                    if (lock !== null) {
                        ...
                        if (changed) {
                            this._locks[blockTmp] = {
                                "state": lock["user"] === this._userId ? 2 : 3,
                                "user": lock["user"],
                                "time": lock["time"],
                                "block": blockTmp,
    ...
    DocsCoApi.prototype.setDocId = function(docid) {
		...
		this.sockjs_url = 'http://192.168.0.180/5.5.1-76/web-apps/apps/documenteditor/main/' + '../../../../doc/' + docid + '/c';
		console.log('this.sockjs_url',this.sockjs_url);
    };
    ...
    DocsCoApi.prototype.auth = function(isViewer, opt_openCmd, opt_isIdle) {
        ...
        if (this._locks) {
            ...
            for (var block in this._locks)
                if (this._locks.hasOwnProperty(block)) {
