...
CPolygon.prototype.fill = function(arrBounds) {
    ...
    for (var LineIndex = nStartLineIndex; LineIndex < CountLines; nStartIndex = 0, LineIndex++) {
        ...
        for (var Index = nStartIndex; Index < arrBounds[LineIndex].length; Index++) {
            ...
            var x1 = Math.round(oBound.X * this.precision),
                x2 = Math.round((oBound.X + oBound.W) * this.precision),
                y1 = Math.round(oBound.Y * this.precision),
                y2 = Math.round((oBound.Y + oBound.H) * this.precision);
...
function CDrawingDocument() {
    ...
    this.DrawMathTrack = function(overlay) {
        ...
        if (null == this.TextMatrix || global_MatrixTransformer.IsIdentity(this.TextMatrix)) {
            ...
            for (nIndex = 0, nCount = SelectPaths.length; nIndex < nCount; nIndex++) {
                ...
                Points = oPath.Points;
                for (nPointIndex = 0; nPointIndex < Points.length - 1; nPointIndex++) {
                    var x = Points[nPointIndex].X,
                        y = Points[nPointIndex].Y;
    ...
    this.SetDrawImagePreviewMargins = function(id, props) {
        ...
        for (var page = 0; page < pageRects.length; page++) {
            ...
            var cur = t;
            while (cur < b) {
                ctx.moveTo(lf, cur + 0.5);
                ctx.lineTo(r, cur + 0.5);

                cur += 2;
                if (cur >= b) break;

                ctx.moveTo(l, cur + 0.5);
                ctx.lineTo(r, cur + 0.5);
    ...
    this.SetDrawImagePreviewBullet = function(id, props, level, is_multi_level, isNoCheckFonts) {
        ...
        if (!is_multi_level) {
            ...
            var y = offset + 2;
            ctx.moveTo(offsetBase, y);
            ctx.lineTo(width_px - offsetBase, y);
            y += (line_w + line_distance);
            ctx.moveTo(offsetBase, y);
            ctx.lineTo(width_px - offsetBase, y);
            y += (line_w + line_distance);
            ...
            ctx.moveTo(text_base_offset_x, y);
            ctx.lineTo(width_px - offsetBase, y);
            y += (line_w + line_distance);
            ctx.moveTo(text_base_offset_x, y);
            ctx.lineTo(width_px - offsetBase, y);
            ...
            for (var i = 0; i < 9; i++) {
                textYs.push({
                    x: text_base_offset_x - ((6.25 * AscCommon.g_dKoef_mm_to_pix) >> 0),
                    y: y + line_w
                });
                if (i == current) {
                    ctx.strokeStyle = "#000000";
    ...
    this.Collaborative_GetTargetPosition = function(UserId) {
        for (var i = 0; i < this.CollaborativeTargets.length; i++) {
            if (UserId == this.CollaborativeTargets[i].Id)
                return {
                    X: this.CollaborativeTargets[i].HtmlElementX,
                    Y: this.CollaborativeTargets[i].HtmlElementY
                };
        }
        ...
        try {
            var _drDocument = {
                ...
                GetVisibleMMHeight: function() {
                    return editor.WordControl.m_oDrawingDocument.GetVisibleMMHeight();
                },
                ...
                GetDotsPerMM: function(v) {
                    return v * g_dKoef_mm_to_pix
                },
                GetMMPerDot: function(v) {
                    return v / this.GetDotsPerMM(1)
                }
            };
            ...
            if (null != _paragraph) {
                _nearpos = {
                    ...
                    ContentPos: _paragraph.Get_ParaContentPos(false, false)
                };
