...
CTable.prototype.Get_PageContentStartPos = function(CurPage, RowIndex, CellIndex) {
    ...
    return {
        X: Pos.X + CellInfo.X_content_start,
    ...
    for (var Index = 0; Index < Count; Index++) {
        ...
        for (var CurCell = 0; CurCell < CellsCount; CurCell++) {
            ...
            if (oFirstPara) {
                ...
                var arrAllParagraphs = oNewCellContent.GetAllParagraphs({
                    All: true
                });
    ...
    for (var nCurRow = 0, nRowsCount = this.GetRowsCount(); nCurRow < nRowsCount; ++nCurRow) {
        ...
        if (oBeforeInfo.Grid > 0)
            arrRowsInfo[nCurRow].push({
                W: this.TableSumGrid[oBeforeInfo.Grid - 1],
                Type: -1,
                GridSpan: 1
            });
    ...
    for (var CurRow = Row_start; CurRow <= Row_last; CurRow++) {
        if (this.RowsInfo[CurRow] && this.RowsInfo[CurRow].Y[nCurPage] && this.RowsInfo[CurRow].H[nCurPage])
            this.Markup.Rows.push({
                Y: this.RowsInfo[CurRow].Y[nCurPage],
                H: this.RowsInfo[CurRow].H[nCurPage]
            });
    }
...
CTable.prototype.private_GetNextCell = function(RowIndex, CellIndex) {
    return this.Internal_Get_NextCell({
        Cell: CellIndex,
        Row: RowIndex
    });
};
CTable.prototype.private_GetPrevCell = function(RowIndex, CellIndex) {
    return this.Internal_Get_PrevCell({
        Cell: CellIndex,
        Row: RowIndex
    });
};
...
CTable.prototype.SetContentSelection = function(StartDocPos, EndDocPos, Depth, StartFlag, EndFlag) {
    ...
    switch (StartFlag) {
        case 0:
            StartRow = StartDocPos[Depth].Position;
            break;
        case 1:
            StartRow = 0;
            isOneElement = false;
            break;
        case -1:
            StartRow = this.Content.length - 1;
            isOneElement = false;
            break;
    }
    ...
    switch (EndFlag) {
        case 0:
            EndRow = EndDocPos[Depth].Position;
            break;
        case 1:
            EndRow = 0;
            isOneElement = false;
    ...
    switch (_StartFlag) {
        case 0:
            StartCell = _StartDocPos[Depth + 1].Position;
            break;
        case 1:
            StartCell = 0;
            break;
        case -1:
            StartCell = this.Content[StartRow].Get_CellsCount() - 1;
            break;
    }
    ...
    switch (_EndFlag) {
        case 0:
            EndCell = _EndDocPos[Depth + 1].Position;
            break;
        case 1:
            EndCell = 0;
            break;
    ...
    var __StartDocPos = _StartDocPos,
...
CTable.prototype.SetContentPosition = function(DocPos, Depth, Flag) {
    ...
    switch (_Flag) {
        case 0:
            CurCell = _DocPos[Depth + 1].Position;
            break;
        case 1:
            CurCell = 0;
            break;
