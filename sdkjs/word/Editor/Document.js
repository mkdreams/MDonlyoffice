...
CStatistics.prototype = {
    ...
    Start: function() {
        ...
        this.PagesId = setTimeout(function() {
            LogicDocument.Statistics_GetPagesInfo();
        }, 1);
...
CSelectedElementsInfo.prototype.RegisterRunWithReviewType = function(nReviewType) {
    switch (nReviewType) {
        case reviewtype_Add:
            this.m_bReviewAdd = true;
            break;
        case reviewtype_Remove:
            this.m_bReviewRemove = true;
            break;
        case reviewtype_Common:
            this.m_bReviewNormal = true;
            break;
    ...
    } else if (e.KeyCode == 188 && true === e.CtrlKey) // Ctrl + ,
    {
        ...
        if (null != TextPr) {
            if (false === this.Document_Is_SelectionLocked(AscCommon.changestype_Paragraph_TextProperties)) {
                ...
                this.AddToParagraph(new ParaTextPr({
                    VertAlign: TextPr.VertAlign === AscCommon.vertalign_SuperScript ? AscCommon.vertalign_Baseline : AscCommon.vertalign_SuperScript
                }));
                this.UpdateInterface();
                this.FinalizeAction();
            }
    ...
    } else if (e.KeyCode == 190 && true === e.CtrlKey) // Ctrl + .
    {
        ...
        if (null != TextPr) {
            if (false === this.Document_Is_SelectionLocked(AscCommon.changestype_Paragraph_TextProperties)) {
                ...
                this.AddToParagraph(new ParaTextPr({
                    VertAlign: TextPr.VertAlign === AscCommon.vertalign_SubScript ? AscCommon.vertalign_Baseline : AscCommon.vertalign_SubScript
                }));
                this.UpdateInterface();
                this.FinalizeAction();
            }
...
CDocument.prototype.Add_NewStyle = function(oStyle) {
    if (false === this.Document_Is_SelectionLocked(AscCommon.changestype_Document_Styles, {
            ...
            Types: [AscCommon.changestype_Paragraph_Properties]
        })) {
...
CDocument.prototype.private_GetRevisionsChangeElement = function(nDirection, oCurrentElement) {
    ...
    if (null === oCurrentElement) {
        oCurrentElement = this.GetCurrentParagraph(false, false, {
            ReturnSelectedTable: true
        });
        if (null === oCurrentElement)
...
CDocument.prototype.AcceptRevisionChangesBySelection = function() {
    ...
    if (null !== CurrentChange) {
        ...
        var SelectedParagraphs = this.GetAllParagraphs({
            Selected: true
        });
...
CDocument.prototype.controller_AddNewParagraph = function(bRecalculate, bForceAdd) {
    ...
    if (type_Paragraph === Item.GetType()) {
        ...
        if (true !== bForceAdd && undefined != Item.GetNumPr() && true === Item.IsEmpty({
                SkipNewLine: true
            }) && true === Item.IsCursorAtBegin()) {
            Item.RemoveNumPr();
            Item.Set_Ind({
                FirstLine: undefined,
                Left: undefined,
            ...
            if (true === Item.IsCursorAtEnd()) {
                ...
                if (LastRun && LastRun.Pr.Lang && LastRun.Pr.Lang.Val) {
                    NewParagraph.SelectAll();
                    NewParagraph.Add(new ParaTextPr({
                        Lang: LastRun.Pr.Lang.Copy()
                    }));
...
CDocument.prototype.AddFieldWithInstruction = function(sInstruction) {
    var oParagraph = this.GetCurrentParagraph(false, false, {
...
CDocument.prototype.GetHyperlinkAnchors = function() {
    ...
    var arrOutline = [];
    this.GetOutlineParagraphs(arrOutline, {
        SkipEmptyParagraphs: true
    });
    var nIndex = 0,
        nCount = arrOutline.length;
...
CDocument.prototype.private_CheckTrackMoveSelection = function(oSelectedInfo) {
    if (!oSelectedInfo)
        oSelectedInfo = this.GetSelectedElementsInfo({
            CheckAllSelection: true
        });
        ...
        if (oParaNumPrPrev) {
            ...
            if (oNum) {
                ...
                if (oAbstractNum === this.AbstractNum) {
                    ...
                    } else if (reviewtype_Add === oReviewType) {} else if (reviewtype_Remove === oReviewType) {
                        if (!oReviewInfo.GetPrevAdded())
                            this.private_UpdateCounter(this.SourceCounter, oNum, oParaNumPrPrev.Lvl);
                    }
                }
