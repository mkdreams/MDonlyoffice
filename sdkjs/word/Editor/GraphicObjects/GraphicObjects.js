...
CGraphicObjects.prototype = {
    ...
    applyZIndex: function(oDrawingObject) {
        var aDrawings = oDrawingObject.aDrawings,
            aZIndex = oDrawingObject.aZIndex;
    ...
    bringForward: function() //перемещаем заселекченные объекты на один уровень выше
    {
        if (this.selection.groupSelection) {
            ...
            var aDrawings = [].concat(this.drawingObjects),
                i, j, aSelect = [],
            ...
            if (false === this.document.Document_Is_SelectionLocked(AscCommon.changestype_None, {
                    Type: changestype_2_ElementsArray_and_Type,
                    CheckType: changestype_Drawing_Props,
                    Elements: oCheckObject.aDrawings
                })) {
    ...
    editChart: function(chart) {
        var chart_space = this.getChartSpace2(chart, null),
            select_start_page, parent_paragraph, nearest_pos;
    ...
    mergeDrawings: function(pageIndex, HeaderDrawings, HeaderTables, FooterDrawings, FooterTables) {
        ...
        var drawings = [],
            tables = [],
            i, hdr_ftr_page = this.graphicPages[pageIndex].hdrFtrPage;
        if (HeaderDrawings) {
    ...
    getFromTargetTextObjectContextMenuPosition: function(oTargetTextObject, pageIndex) {
        var dPosX = this.document.TargetPos.X,
            dPosY = this.document.TargetPos.Y,
            oTransform = oTargetTextObject.transformText;
        return {
            X: oTransform.TransformPointX(dPosX, dPosY),
            Y: oTransform.TransformPointY(dPosX, dPosY),
            PageIndex: this.document.TargetPos.PageNum
        };
    },
    ...
    getLeftTopSelectedObject: function(pageIndex) {
        if (this.selectedObjects.length > 0) {
            ...
            aSelectedObjectsCopy.sort(function(a, b) {
