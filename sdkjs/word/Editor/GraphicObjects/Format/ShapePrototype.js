    ...
    if (this.recalcInfo && this.recalcInfo.recalculateTransform) {
        if (this.group) {
            ...
            if (oMainGroup.parent && oMainGroup.parent.Refresh_RecalcData) {
                oMainGroup.parent.Refresh_RecalcData({
                    Type: AscDFH.historyitem_Drawing_SetExtent
                });
            }
        } else {
...
CShape.prototype.recalculateShapeStyleForParagraph = function() {
    this.textStyleForParagraph = {
        TextPr: g_oDocumentDefaultTextPr.Copy(),
        ParaPr: g_oDocumentDefaultParaPr.Copy()
    };
    ...
    if (HdrFtr)
        ...
        if (!this.group) {
            ...
            if (isRealObject(cur_group.parent) && cur_group.parent.Refresh_RecalcData2)
                cur_group.parent.Refresh_RecalcData2({
                    Type: AscDFH.historyitem_Drawing_SetExtent
                });
        }
    }
