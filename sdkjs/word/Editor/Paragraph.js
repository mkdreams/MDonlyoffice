            ...
            while (null != Element) {
                if (!pGraphics.DrawTextArtComment) {
                    if (Element.Additional.Active === true) {
                        ...
                        pGraphics.b_color1(255, 0, 0, 150);
                        pGraphics.trangle(Element.x1, Element.y1, Element.y1 - Element.y0, Element.y1 - Element.y0, true);
                        pGraphics.df();

                        pGraphics.b_color1(255, 0, 0, 150);
                        pGraphics.trangle(Element.x0, Element.y1, Element.y1 - Element.y0, Element.y1 - Element.y0, false);
                        pGraphics.df();
                    } else {
        ...
        if (true != bShift) {
            if (nFirst < -0.001) {
                ...
                if (nLeft + nFirst < nCurTabPos - 0.001) {
                    this.Set_Ind({
                        FirstLine: (((nFirst + nLeft) / nDefaultTabStop + 0.001) | 0) * nDefaultTabStop - nLeft
                    }, false);
                } else {
                    if (Math.abs((((nFirst + nLeft) / nDefaultTabStop + 0.001) | 0) * nDefaultTabStop - (((nLeft) / nDefaultTabStop + 0.001) | 0) * nDefaultTabStop) < 0.001 && nFirst < nDefaultTabStop)
                        this.Set_Ind({
                            FirstLine: ((((nFirst + nLeft) / nDefaultTabStop + 0.001) | 0) + 1) * nDefaultTabStop - nLeft
                        }, false);
                    else
                        this.Set_Ind({
                            Left: ((((nFirst + nLeft) / nDefaultTabStop + 0.001) | 0) + 1) * nDefaultTabStop - nFirst
                        }, false);
                }
            }
        } else {
            if (Math.abs(nFirst) < 0.001) {
                if (Math.abs(nLeft) < 0.001)
                    ...
                    if (Math.abs(nCurTabPos - nLeft) < 0.001)
                        this.Set_Ind({
                            Left: (((nLeft / nDefaultTabStop + 0.001) | 0) - 1) * nDefaultTabStop
                        }, false);
                    else
                        this.Set_Ind({
                            Left: nCurTabPos
                ...
                if (Math.abs(nFirst + nLeft) < 0.001) {
                    ...
                    if (Math.abs(nLeft + nFirst - nCurTabPos) < 0.001) {
                        var nPrevTabPos = Math.max(0, ((((nFirst + nLeft) / nDefaultTabStop + 0.001) | 0) - 1) * nDefaultTabStop);
                        this.Set_Ind({
                            Left: nPrevTabPos - nFirst
                        }, false);
                    } else {
                        this.Set_Ind({
                            Left: nCurTabPos - nFirst
                        }, false);
                    }
                }
            }
...
Paragraph.prototype.Can_IncreaseLevel = function(bIncrease) {
    var CurLevel = AscFormat.isRealNumber(this.Pr.Lvl) ? this.Pr.Lvl : 0,
        NewPr, OldPr = this.Get_CompiledPr2(false).TextPr,
        DeltaFontSize, i, j, RunPr;
    if (bIncrease) {
        if (CurLevel >= 8) {
            return false;
        }
    ...
    if (false !== CorrectAnchor) {
        ...
        if (true === this.Selection_CheckParaEnd()) {
            ...
            for (var Pos = 0; Pos <= StartPos; Pos++) {
                if (false === this.Content[Pos].IsSelectedAll({
                        SkipAnchor: true
                    })) {
            ...
            for (var Pos = 0; Pos < _StartPos; Pos++) {
                if (true !== this.Content[Pos].IsEmpty({
                        SkipAnchor: true
                    })) {
            ...
            for (var Pos = nStartPos, nParaPos = 0; Pos <= nEndPos; Pos++) {
                ...
                if ((nStartPos === Pos || nEndPos === Pos) && true !== Item.IsSelectedAll()) {
                    ...
                    if (Item.Type !== para_RevisionMove) {
                        oPara.Internal_Content_Add(nParaPos, Item.Copy(false, {
                            CopyReviewPr: true
                        }), false);
                        nParaPos++;
                    }
    ...
    for (var CurPos = StartPos; CurPos <= EndPos; CurPos++) {
        ...
        if (true === DrawingLayout.Layout) {
            ...
            return {
                ...
                PageLimitsOrigin: PageLimitsOrigin
            };
        }
    }
...
Paragraph.prototype.GetOutlineParagraphs = function(arrOutline, oPr) {
    if (this.IsEmpty({
        ...
        }) && (!oPr || false !== oPr.SkipEmptyParagraphs))
        return;
    ...
    if (undefined !== nOutlineLvl &&
        ...
        for (var nIndex = 0, nCount = oPr.Styles.length; nIndex < nCount; ++nIndex) {
            if (oPr.Styles[nIndex].Name === sStyleName)
                return arrOutline.push({
                    Paragraph: this,
                    Lvl: oPr.Styles[nIndex].Lvl - 1
                });
        }
    }
