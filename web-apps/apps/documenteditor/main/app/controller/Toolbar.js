    ...
    DE.Controllers.Toolbar = Backbone.Controller.extend(_.extend({
        ...
        onComboOpen: function(needfocus, combo) {
            _.delay(function() {
                ...
                else if (!combo.isMenuOpen()) input.one('mouseup', function(e) {
                    e.preventDefault();
                });
            }, 10);
        },
        ...
        onColorSchemaShow: function(menu) {
            if (this.api) {
                ...
                var item = _.find(menu.items, function(item) {
        ...
        onSaveStyle: function(style) {
            ...
            if (me.api) {
                ...
                _.each(window.styles.get_MergedStyles(), function(style) {
                    formats.push({
                        value: style,
                        displayValue: mainController.translationTable[style.get_Name()] || style.get_Name()
                    })
                });
        ...
        _onInitEditorStyles: function(styles) {
            ...
            if (listStyles.menuPicker.store.length > 0 && listStyles.rendered) {
                var styleRec;
                if (self._state.prstyle) styleRec = listStyles.menuPicker.store.findWhere({
                    title: self._state.prstyle
                });
        ...
        onHomeOpen: function() {
            ...
            if (listStyles && listStyles.needFillComboView && listStyles.menuPicker.store.length > 0 && listStyles.rendered) {
                var styleRec;
                if (this._state.prstyle) styleRec = listStyles.menuPicker.store.findWhere({
                    title: this._state.prstyle
                });
                listStyles.fillComboView((styleRec) ? styleRec : listStyles.menuPicker.store.at(0), true);
            }
        },
        ...
        onApiCoAuthoringDisconnect: function(enableDownload) {
            this.mode.isEdit && this.toolbar.setMode({
                isDisconnected: true,
        ...
        onAppShowed: function(config) {
            ...
            me.toolbar.render(_.extend({
                isCompactView: compactview
            }, config));

            var tab = {
                action: 'review',
                caption: me.toolbar.textTabCollaboration
            };
            ...
            if (config.isEdit) {
                ...
                if (config.isDesktopApp) {
                    if (config.canProtect) {
                        tab = {
                            action: 'protect',
                            caption: me.toolbar.textTabProtect
                        };
