    ...
    Common.Views.History = Common.UI.BaseView.extend(_.extend({
        ...
        render: function(el) {
            ...
            $(el).html(this.template({
                scope: this
            })).width((parseInt(Common.localStorage.getItem('de-mainmenu-width')) || MENU_SCALE_PART) - SCALE_MIN);
            ...
            var changetooltip = function(dataview, view, record) {
                if (record.get('isRevision')) {
                    ...
                    var btns = $(view.el).find('.revision-expand').tooltip({
                        title: (record.get('isExpanded')) ? me.textHide : me.textShow,
                        placement: 'cursor'
                    });
                    if (btns.length > 0)
