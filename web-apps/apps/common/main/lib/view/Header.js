    ...
    Common.Views.Header = Backbone.View.extend(_.extend(function() {
            ...
            if ($userList) {
                if (usercount > 1 || usercount > 0 && appConfig && !appConfig.isEdit && !appConfig.isRestrictedEdit) {
                    $userList.html(templateUserList({
                        users: collection.chain().filter(function(item) {
                            return item.get('online') && !item.get('view')
                        }).groupBy(function(item) {
                            return item.get('idOriginal');
                        }).value(),
        ...
        function applyUsers(count, originalCount) {
            ...
            $btnUsers.find('.caption')
                .css({
                    'font-size': ((has_edit_users) ? '12px' : '14px'),
        ...
        function onAppReady(mode) {
            ...
            $panelUsers.on('shown.bs.dropdown', function() {
                $userList.scroller && $userList.scroller.update({
                    minScrollbarLength: 40,
                    alwaysVisibleY: true
                });
            });
        ...
        return {
            ...
            getPanel: function(role, config) {
                ...
                if (role == 'title') {
                    ...
                    me.labelDocName.text = function(str) {
                        this.val(str);
                    }; // redefine text function to lock temporaly rename docuemnt option
                    me.labelDocName.text(me.documentCaption);
            ...
            setSaveStatus: function(status) {
                if ($saveStatus) {
                    ...
                    switch (status) {
                        case 'begin':
                            _text = this.textSaveBegin;
                            break;
                        case 'changed':
                            _text = this.textSaveChanged;
                            break;
                        default:
                            _text = this.textSaveEnd;
                    }
