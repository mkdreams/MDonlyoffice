...
function patchDropDownKeyDown(e) {
    ...
    if (e.keyCode == 39) { // right
        ...
        if (e.keyCode == 38) {
            index > 0 ? index-- : ($this.hasClass('no-cyclic') ? (index = 0) : (index = $items.length - 1));
        } else // up
            if (e.keyCode == 40) {
                index < $items.length - 1 ? index++ : ($this.hasClass('no-cyclic') ? (index = $items.length - 1) : (index = 0));
            } // down
        if (!~index) index = 0;
 ...
 * */
(function() {
    var eventsObj = $._data($(document).get(0), "events"),
        clickDefHandler;
