...
define([
    ...
    Common.UI.DataView = Common.UI.BaseView.extend({
        ...
        render: function(parentEl) {
            ...
            if (!this.rendered) {
                ...
                if (this.enableKeyEvents && this.parentMenu && this.handleSelect) {
                    if (!me.showLast)
                        this.parentMenu.on('show:before', function(menu) {
                            me.deselectAll();
                        });
        ...
        deselectAll: function(suspendEvents) {
            console.log('deselectAll', suspendEvents);
            if (suspendEvents)
                this.suspendEvents();
        ...
        getSelectedRec: function() {
            return (this.multiSelect) ? this.store.where({
                selected: true
            }) : this.store.findWhere({
                selected: true
            });
        },
