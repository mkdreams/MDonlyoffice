...
define([
    ...
    var Desktop = function() {
            ...
            window.on_native_message = function(cmd, param) {
                ...
                if (/window:features/.test(cmd)) {
                    ...
                    if (obj.canUndock == 'true') {
                        if (!config.canUndock) {
                            ...
                            if (!_.isEmpty(config))
                                Common.NotificationCenter.trigger('app:config', {
                                    canUndock: true
                                });
                        }
                    }
                ...
                if (/window:status/.test(cmd)) {
                    ...
                    if (obj.action == 'undocking') {
                        Common.NotificationCenter.trigger('undock:status', {
                            status: obj.status == 'undocked' ? 'undocked' : 'docked'
                        });
                    }
                } else
                    ...
                    if (param == 'request') {
                        ...
                        if (!config.callback_editorconfig) {
                            config.callback_editorconfig = function() {
                                setTimeout(function() {
                                    window.on_native_message(cmd, param);
                                }, 0);
                            }
                        }
                    }
        ...
        var _onTitleButtonDisabled = function(action, e, status) {
            ...
            native.execCommand('title:button', JSON.stringify({
                disabled: _buttons
            }));
        };
        ...
        return {
            init: function(opts) {
                ...
                if (config.isDesktopApp) {
                    ...
                    Common.NotificationCenter.on('action:undocking', function(opts) {
                        native.execCommand('editor:event', JSON.stringify({
                            action: 'undocking',
                            state: opts == 'dock' ? 'dock' : 'undock'
                        }));
                    });
            ...
            requestClose: function() {
                if (config.isDesktopApp && !!native) {
                    native.execCommand('editor:event', JSON.stringify({
                        action: 'close',
                        url: config.customization.goback.url
                    }));
                }
            }
        };
