...
define([
    ...
    Common.Controllers.Plugins = Backbone.Controller.extend(_.extend({
        ...
        configPlugins: {
            autostart: []
        }, // {config: 'from editor config', plugins: 'loaded plugins', UIplugins: 'loaded customization plugins', autostart: 'autostart guids'}
        serverPlugins: {
            autostart: []
        }, // {config: 'from editor config', plugins: 'loaded plugins', autostart: 'autostart guids'}
        collections: [
                ...
                'Toolbar': {
                    'render:before': function(toolbar) {
                        ...
                        if (!appOptions.isEditMailMerge && !appOptions.isEditDiagram) {
                            var tab = {
                                action: 'plugins',
                                caption: me.panelPlugins.groupCaption
                            };
            ...
            Common.NotificationCenter.on({
                'layout:resizestart': function(e) {
                    if (me.panelPlugins.isVisible()) {
                        ...
                        me._moveOffset = {
                            x: offset.left + parseInt(me.panelPlugins.currentPluginFrame.css('padding-left')),
        ...
        onPluginMouseUp: function(x, y) {
            if (this.pluginDlg) {
                ...
                Common.NotificationCenter.trigger('frame:mouseup', {
                    pageX: x * Common.Utils.zoom() + this._moveOffset.x,
                    pageY: y * Common.Utils.zoom() + this._moveOffset.y
                });
        },
        ...
        onPluginMouseMove: function(x, y) {
            if (this.pluginDlg) {
                ...
                if (this.pluginDlg.binding.drag) this.pluginDlg.binding.drag({
                    pageX: x * Common.Utils.zoom() + offset.left,
                    pageY: y * Common.Utils.zoom() + offset.top
                });
                if (this.pluginDlg.binding.resize) this.pluginDlg.binding.resize({
                    pageX: x * Common.Utils.zoom() + offset.left,
                    pageY: y * Common.Utils.zoom() + offset.top
                });
            } else
                Common.NotificationCenter.trigger('frame:mousemove', {
                    pageX: x * Common.Utils.zoom() + this._moveOffset.x,
                    pageY: y * Common.Utils.zoom() + this._moveOffset.y
                });
        },
