...
(function($) {
    ...
    Tip.prototype = $.extend({}, $.fn.tooltip.Constructor.prototype, {
                        ...
                        switch (placementEx[2]) {
                            case 'left':
                                ...
                                if (this.$element.outerWidth() <= 18) {
                                    tp.left -= 4;
                                } // if button is small move tooltip left
                                break;
                            case 'right':
        ...
        enter: function(obj) {
            ...
            if ($target.is('[role=menu]') || $target.parentsUntil(obj.currentTarget, '[role=menu]').length && obj.target !== obj.currentTarget || this.tip().is(':visible')) {
                return;
            }
